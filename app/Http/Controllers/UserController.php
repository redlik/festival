...
class UserController extends Controller
{
    ...
    {
        $events = Event::where('user_id', Auth::id())->orderBy('start_date', 'asc')
            ->withCount(['attendee' => function ($q) {
                    $q->where('waiting_status', false);
            },
                'waiting' => function ($q) {
                    $q->where('waiting_status', true);
                }
                ])
            ->with('attendee', 'venue', 'user.organiser')->get();
